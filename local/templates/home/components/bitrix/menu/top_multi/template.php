...
foreach($arResult as $arItem):?>
	...
	<?if ($arItem["IS_PARENT"]):?>
        ...
        <li class="has-children <?if ($arItem["SELECTED"]): echo 'active'; endif?>"><a href="<?=$arItem["LINK"]?>"><?=$arItem["TEXT"]?></a>
            <ul class="dropdown">
		<?else:?>
                <li class="has-children <?if ($arItem["SELECTED"]): echo 'active'; endif?>"><a href="<?=$arItem["LINK"]?>"><?=$arItem["TEXT"]?></a>
                    <ul class="dropdown">
	...
	<?else:?>
		...
		<?if ($arItem["PERMISSION"] > "D"):?>
			...
			<?if ($arItem["DEPTH_LEVEL"] == 1):?>
                <li class="<?if ($arItem["SELECTED"]):?>active<?endif?>"\><a href="<?=$arItem["LINK"]?>" class="<?if ($arItem["SELECTED"]):?>active<?endif?>"><?=$arItem["TEXT"]?></a></li>
            <?else:?>
